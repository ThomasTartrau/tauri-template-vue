...
const formSchema = toTypedSchema(
  z.object({
    new_password: z.string().min(1, t('reset_password_page.new_password.errors.too_short')),
    confirm_password: z.string().min(1, t('reset_password_page.confirm_password.errors.too_short')),
  }),
)
  ...
  <div v-if="token" class="flex items-center justify-center min-h-screen">
    <Card class="mx-auto">
      <CardHeader>
        ...
        <CardDescription>{{ t('reset_password_page.card.description') }}</CardDescription>
      </CardHeader>
      <CardContent>
        <form @submit.prevent="onSubmit">
          <div class="mb-4">
            <FormField v-slot="{ componentField }" name="new_password">
              <FormItem v-auto-animate>
                <FormLabel>{{ t('reset_password_page.new_password.label') }}</FormLabel>
                <FormControl>
                  <Input
                    type="password"
                    placeholder="************"
          ...
          <div class="mb-4">
            ...
            >
              <FormItem v-auto-animate>
                <FormLabel>{{ t('reset_password_page.confirm_password.label') }}</FormLabel>
                <FormControl>
                  <Input
                    type="password"
                    placeholder="************"
